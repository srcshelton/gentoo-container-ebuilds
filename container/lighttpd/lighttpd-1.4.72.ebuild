...
#IUSE="+brotli dbi gnutls kerberos ldap +lua maxminddb mbedtls mmap mysql +nettle nss +pcre php postgres rrdtool sasl selinux sqlite ssl systemd test unwind webdav xattr +zlib zstd"
IUSE="php"
#RESTRICT="!test? ( test )"
...
src_prepare() {
	...
	if use php; then
		...
		initd="lighttpd+php.initd-r1_common"
		regex="s#@PVR@#${PVR}# ; s#@PPV@#${PHP_TARGETS##* }#"
	else
		initd="lighttpd.initd-r1_common"
		regex="s#@PVR@#${PVR}#"
	fi
	for f in "${initd}"; do
		sed -e "${regex}" \
