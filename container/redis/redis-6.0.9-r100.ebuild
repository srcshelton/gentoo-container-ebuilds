...
src_prepare() {
	...
	for f in redis.initd-5_common redis-sentinel.initd_common; do
		sed \
			-e "s#@PVR@#${PVR}#" \
...
src_install() {
	...
	newinitd "${T}/redis.initd-5_common" redis

	newconfd "${FILESDIR}/redis-sentinel.confd" redis-sentinel
	newinitd "${T}/redis-sentinel.initd_common" redis-sentinel
